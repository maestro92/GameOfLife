...
/*
  ...
  */
    ...
    // FragColor = texelFetchOffset(u_sourceTexture, T, 0, o);
	// FragColor = texelFetch(u_sourceTexture, T, 0);
	// ivec2 oW1 = ivec2(-1, 0);
	// FragColor = texelFetchOffset(u_sourceTexture, T, 0, oW);

/*
	float vx = gl_FragCoord.x / 14;
	float vy = gl_FragCoord.y / 12;

	FragColor = vec4(vx, vy, 1.0, 1.0);
*/
